...
- 阶段 3 自动化门禁：`npm test` 32 个测试通过；`npx tsc --noEmit`、`npm run build` 与 `git diff --check` 通过；主 JS 801.00 kB，gzip 247.29 kB
- 阶段 3 人工回归：用户于 2026-06-11 确认验收通过
...
- 阶段 6 自动化门禁：`npm test` 48 个测试通过；`npx tsc --noEmit`、`npm run build` 与 `git diff --check` 通过；主 JS 778.58 kB，gzip 240.85 kB
- 阶段 6 人工回归：用户于 2026-06-12 确认验收通过
- 阶段 7 自动化门禁：`npm test` 53 个测试通过；`npx tsc --noEmit`、`npm run build` 与 `git diff --check` 通过；主 JS 780.17 kB，gzip 241.83 kB
- 阶段 7 人工回归：用户于 2026-06-12 确认验收通过
...
| 6. UI 状态迁移 | [06-UI-STATE-MIGRATION.md](./06-UI-STATE-MIGRATION.md) | 已完成 | 3-5 天 | 本次执行约 0.1 小时，不含人工回归 | 阶段 5 |
| 7. 组件拆分 | [07-COMPONENT-SPLITTING.md](./07-COMPONENT-SPLITTING.md) | 已完成 | 6-9 天 | 本次执行约 0.3 小时，不含人工回归 | 阶段 6 |
...
| 富文本预览逻辑复杂 | 主列表预览维护成本较高 | 先提取纯函数并补测试，再拆组件 |
...
| 2026-06-12 | 阶段 6 跨组件 UI 状态迁移人工验收通过，启动阶段 7 | UI 状态迁移已满足阶段门禁；阶段 7 不恢复已移除的紧凑预览窗口 |
| 2026-06-12 | 阶段 7 高复杂度组件拆分人工验收通过 | 组件拆分已满足阶段门禁，阶段 8 尚未启动 |
