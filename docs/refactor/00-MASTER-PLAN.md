...
- 完成时间：待完成
- 依赖阶段：无
- 当前阶段：阶段 8 Less 接入、样式隔离与债务清理实现完成，待人工回归；临时任务“彻底移除富文本支持”仍待人工回归
- 阻塞项：无
...
- 阶段 8 实施前基线：`npm test` 48 个测试通过；`npx tsc --noEmit`、`npm run build` 通过；构建耗时 8.14 秒；CSS 125.38 kB，主 JS 753.22 kB
- 阶段 8 自动化门禁：`npm test` 48 个测试通过；`npx tsc --noEmit`、`npm run build` 与 `git diff --check` 通过；构建耗时 8.79 秒；CSS 151.60 kB，主 JS 702.66 kB；NSIS 工具下载失败，未生成安装包
...
- 插入时间：2026-06-12
- 状态：实施完成，待人工回归
- 原因：现有富文本捕获、预览和带格式粘贴支持不完整，继续维护会扩大复杂度与回归面。
- 范围：删除富文本捕获、展示、带格式粘贴、设置、模型和 IPC 支持；右键历史条目保留粘贴行为，但统一为纯文本粘贴。
- 数据迁移：升级时幂等重建 `clipboard_history`，将既有 `rich_text` 转为 `text`，保留纯文本内容与条目元数据，删除 `html_content` 字段和数据。
- 兼容性：迁移不可逆，升级后的数据库不再兼容旧版本应用；回滚代码前必须恢复升级前数据库备份。
- 阶段关系：任务独立于阶段 8，不启动或混入样式工作。
...
| 8. Less、样式与清理 | [08-STYLES-AND-CLEANUP.md](./08-STYLES-AND-CLEANUP.md) | 实现完成，待人工回归 | 4-7 天 | 本次执行约 0.3 小时，不含人工回归 | 阶段 7 |
...
| 富文本数据库迁移不可逆 | 升级后旧版本无法读取新数据库结构 | 明确记录旧版本不兼容，回滚依赖升级前外部备份 |
...
| 2026-06-12 | 阶段 7 高复杂度组件拆分人工验收通过，启动阶段 8 | 组件拆分已满足阶段门禁；阶段 8 保持 DOM、类名和主题 Token 不变 |
| 2026-06-12 | 临时插入“彻底移除富文本支持”任务 | 富文本支持不完整且维护成本高；批准不可逆迁移并接受旧版本数据库不兼容 |
